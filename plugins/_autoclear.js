  ...
  for (const [name, path] of Object.entries(paths)) {
    ...
    fs.readdir(path, (err, subbotDirs) => {
      ...
      subbotDirs.forEach((subbotDir) => {
        ...
        fs.readdir(subbotPath, (err, files) => {
          ...
          Promise.all(deletePromises).then(() => {
            if (filesDeleted > 0) {
              console.log(`Se eliminaron ${filesDeleted} archivos de la sesión Jadibots: ${subbotDir}`);
            }
          }).catch((err) => {
      ...
      if (totalFilesDeleted === 0) {
        ...
        console.log(`Se eliminaron un total de ${totalFilesDeleted} archivos de todas las sesiones Jadibots en ${name}`);
      }
    });
  }
}
...
function cleanSessions() {
  const sessionPath = paths.Sessions;
